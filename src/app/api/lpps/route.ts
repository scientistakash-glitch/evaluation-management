...
export async function POST(request: NextRequest) {
  try {
    ...
    const { ptatId, name, code, duration, fee, description, totalSeats, categoryWiseSeats } = body;
    if (!ptatId || !name || !code || duration == null) {
      return NextResponse.json({ error: 'ptatId, name, code, and duration are required' }, { status: 400 });
    }
    const lpp = await createLpp({ ptatId, name, code, duration, fee: fee ?? 0, description, totalSeats: totalSeats ?? 0, categoryWiseSeats: categoryWiseSeats ?? {} });
